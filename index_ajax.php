...
$link = mysqli_connect("localhost", "root", "yeeeeeee", "masteryi") // 建立MySQL的資料庫連結
...
if ($oper == "query") {
      ...
      if ($result = mysqli_query($link, $sql)) {
            while ($row = mysqli_fetch_assoc($result)) {
                  $a['data'][] = array($row["name"], $row["account"], $row["password"], $row["tmp_password"],$row["start_time"],$row["end_time"]);
            }
